      ...
      <div className='mx-auto w-full max-w-[1180px] px-8 py-16'>
          ...
          <div>
            <div className='mb-5 flex items-center gap-3'>
              ...
              <div className='flex flex-col leading-tight'>
                ...
                <span className='text-[11.5px] uppercase tracking-[0.08em] text-ink-500'>Modern transfer agency</span>
              </div>
            </div>
            <h1 className='max-w-2xl text-[40px] font-semibold leading-[1.1] tracking-[-0.03em] text-ink-900'>
              Regulated shareholder infrastructure,
              <span className='text-brand-700'> rebuilt software-first.</span>
            </h1>
            <p className='mt-4 max-w-xl text-[14px] text-ink-600'>
              Auditable, immutable, AI-guided workflows for transfers, issuance, dividends, proxy voting, and corporate actions. Humans in
              the loop where it matters.
            </p>
          </div>
          <div className='flex flex-col items-end gap-2 text-[12px] text-ink-500'>
        ...
        <section className='grid grid-cols-1 gap-4 md:grid-cols-2'>
          {PORTAL_ORDER.map(id => {
            ...
            return (
              <Link
                className='group relative flex flex-col rounded-[14px] border border-line bg-surface p-6 shadow-xs transition hover:border-border-strong hover:shadow-md'
                href={portal.href}
                key={id}
              >
                <div className='flex items-center gap-2'>
                  <span className={`portal-dot ${id}`} />
                  <span className='text-[11.5px] font-semibold uppercase tracking-[0.08em] text-ink-500'>{portal.role}</span>
                </div>
                <div className='mt-3 flex items-center justify-between gap-2'>
                  <h2 className='text-[20px] font-semibold text-ink-900'>{meta.name}</h2>
                  <span className='flex h-7 w-7 items-center justify-center rounded-full border border-line text-ink-500 transition group-hover:border-ink-900 group-hover:bg-ink-900 group-hover:text-white'>
                    <Icon name='arrow-right' size={14} />
                  </span>
                </div>
                <p className='mt-2 text-[13.5px] text-ink-600'>{portal.blurb}</p>
                <ul className='mt-5 flex flex-col gap-1.5 border-t border-line pt-4 text-[13px] text-ink-700'>
        ...
        <footer className='mt-12 flex items-center justify-between text-[12px] text-ink-500'>
