...
export function GuidedIntake() {
  ...
  const [destinationType, setDestinationType] = useState<'broker' | 'certificate' | 'entity' | 'individual' | 'joint' | 'trust'>('broker')
  ...
  const draftCaseId = 'TR-DRAFT-120502'
  const onSuccessStep = step === 'success'
  const progressPct = Math.round((Math.min(stepIdx, 5) / 5) * 100)
      ...
      {!onSuccessStep && (
        <div className='flex flex-wrap items-center gap-x-4 gap-y-1 rounded-md border border-line bg-surface-2 px-3 py-2 text-[11.5px] text-ink-500'>
          <span className='flex items-center gap-1.5'>
            ...
            <span className='num font-semibold text-ink-800'>{draftCaseId}</span>
          </span>
            ...
            <div className='panel-body pt-5'>
              ...
              {step === 'holding' && (
                <div className='flex flex-col gap-4'>
                  ...
                  <div className='flex flex-col gap-2'>
                    <div className='text-[12px] font-medium text-ink-700'>Transfer scope</div>
                    <div className='inline-flex rounded-md border border-line bg-white p-0.5 self-start'>
                      <button
                        className={`px-3 py-1.5 text-[12.5px] font-medium rounded-sm ${scope === 'full' ? 'bg-surface-sunken text-ink-900' : 'text-ink-500'}`}
                        onClick={() => {
                          setScope('full')
                          setShares('1240')
                        }}
                      ...
                      <button
                        className={`px-3 py-1.5 text-[12.5px] font-medium rounded-sm ${scope === 'partial' ? 'bg-surface-sunken text-ink-900' : 'text-ink-500'}`}
                  ...
                  <div>
                    ...
                    <div className='mt-1 text-[11.5px] text-ink-500'>
                      {scope === 'full'
                        ? 'All 1,240 shares will be transferred — this closes the position.'
              ...
              {step === 'destination' && (
                <div className='flex flex-col gap-4'>
                  ...
                  <div className='flex flex-col gap-2'>
                    <div className='text-[12px] font-medium text-ink-700'>Destination registration type</div>
                    <div className='grid grid-cols-2 gap-2 md:grid-cols-3'>
                      {(
                        [
                          { id: 'broker', label: 'Brokerage account', icon: 'landmark' },
                          { id: 'individual', label: 'Individual (re-registration)', icon: 'user-round' },
                          { id: 'joint', label: 'Joint tenants', icon: 'users' },
                          { id: 'trust', label: 'Trust', icon: 'shield' },
                          { id: 'entity', label: 'Entity / LLC', icon: 'building' },
                          { id: 'certificate', label: 'Physical certificate', icon: 'file-text' },
                        ] as const
                      ).map(t => (
                        <label
                        ...
                        >
                          <input
                            ...
                            type='radio'
                          />
                          <Icon className='text-brand-700' name={t.icon} size={13} />
                ...
                <div className='flex flex-col gap-3'>
                  ...
                  <div className='flex flex-col gap-2'>
                        ...
                        [
                          {
                            id: 'medallion',
                            label: 'Medallion guarantee',
                            blurb: 'STA / MSP / SEMP surety stamp',
                          },
                          {
                            id: 'waiver',
                            label: 'Under-threshold waiver',
                            blurb: 'Transfer value is under $25,000',
                          },
                          {
                            id: 'affidavit',
                            label: 'Affidavit of loss',
                            blurb: 'Only for lost-certificate replacement',
                          },
                        ] as const
                      ).map(p => (
                        <label
                    ...
                    {medallionPath !== 'medallion' && (
                      <div className='text-[11.5px] text-warning-700'>
              ...
              {step === 'success' && (
                <div className='flex flex-col items-center gap-3 py-6 text-center'>
                  ...
                  <div className='flex gap-2'>
                    ...
                    <Link className='btn btn-brand btn-sm' href='/investor/transfers/TR-120491'>
                      View status tracker
