...
const brokers = ['Charles Schwab & Co., Inc.', 'Fidelity Brokerage Services LLC', 'Interactive Brokers LLC', 'Robinhood Securities, LLC', 'Vanguard Brokerage']
  ...
  const reviewSections: ReviewSection[] = [
    ...
    },
    {
      fields: [
        ...
        { label: 'Shares', value: <span className='num'>{shareCount.toLocaleString('en-US')}</span>, hint: scope === 'full' ? 'Full position' : `of ${activeHolding.available.toLocaleString('en-US')} available` },
        {
          hint: activeHolding.restriction ? 'Restricted — reviewer sign-off required.' : 'No restrictions detected.',
          label: 'Restrictions',
          value: activeHolding.restriction ? <Badge tone='warning'>{activeHolding.restriction}</Badge> : <Badge tone='positive'>None</Badge>,
        },
      ],
      icon: 'user-round',
      onEdit: () => go('holding'),
    ...
    {
      fields: [
        { label: 'Effective date', value: new Date(effectiveDate).toLocaleDateString('en-US', { day: 'numeric', month: 'short', year: 'numeric' }) },
    ...
    {
      fields: [
        { hint: 'Generated by Proxi · ready to sign.', label: 'Stock power', value: uploaded['stock-power'] ? <Badge tone='positive'>Uploaded</Badge> : <Badge tone='danger'>Missing</Badge> },
        { hint: `Path: ${medallionPath}`, label: 'Signature guarantee', value: uploaded.medallion ? <Badge tone='positive'>Uploaded</Badge> : <Badge tone='danger'>Missing</Badge> },
    ...
    },
    {
      fields: [
        ...
        { label: 'Ledger impact', value: `−${shareCount.toLocaleString('en-US')} from source · +${shareCount.toLocaleString('en-US')} to destination` },
            ...
            <div className='panel-header'>
              <div>
                ...
                <div className='panel-subtitle'>
                  {step === 'success' ? 'All done · tracker is live' : `Step ${Math.min(stepIdx + 1, totalSteps)} of ${totalSteps} · Guided intake`}
                </div>
              </div>
            ...
            <div className='panel-body pt-5'>
                ...
                <div className='flex flex-col gap-4'>
                  ...
                  <div className='grid grid-cols-1 gap-3 md:grid-cols-2'>
                    <label className='text-[12px] font-medium text-ink-700'>
                      <span>
                        Shares to transfer <span aria-hidden className='text-danger-700'>*</span>
                    ...
                    <label className='text-[12px] font-medium text-ink-700'>
                      <span>
                        Effective date <span aria-hidden className='text-danger-700'>*</span>
                        <span className='sr-only'>required</span>
                      </span>
                      <input
              ...
              {step === 'destination' && (
                <div className='flex flex-col gap-4'>
                  ...
                  <div className='grid grid-cols-1 gap-3 md:grid-cols-2'>
                    <label className='text-[12px] font-medium text-ink-700'>
                      <span>
                        Destination broker <span aria-hidden className='text-danger-700'>*</span>
                        <span className='sr-only'>required</span>
                      </span>
                    ...
                    <label className='text-[12px] font-medium text-ink-700'>
                      <span>
                        Destination account # <span aria-hidden className='text-danger-700'>*</span>
                        <span className='sr-only'>required</span>
                      </span>
                      <input
                    ...
                    <label className='text-[12px] font-medium text-ink-700'>
                      <span>
                        Account title <span aria-hidden className='text-danger-700'>*</span>
                        <span className='sr-only'>required</span>
                      </span>
              ...
              {step === 'success' && (
                <TransferConfirmationScreen
                  ...
                  nextSteps={[
                    { description: 'Proxi scans documents and re-runs validation against live ledger data.', icon: 'sparkles', title: 'AI pre-check' },
                    { description: 'A licensed transfer-agent reviewer confirms evidence and approves the transfer.', icon: 'shield-check', title: 'Reviewer sign-off' },
                    { description: 'Shares move on the shareholder ledger. You get a ledger-posted confirmation.', icon: 'check-circle', title: 'Ledger posting' },
                  ]}
...
function PrimaryStepCta({
  ...
  const config = (() => {
    switch (step) {
      case 'confirm':
        return { disabled: blockers > 0, icon: 'check', label: blockers > 0 ? `Fix ${blockers} blocker${blockers === 1 ? '' : 's'}` : 'Submit transfer for review' }
      case 'destination':
