...
import { Avatar, Badge, type StatusTone, Tabs } from '@/components/ui'
...
const items: InboxItem[] = [
  {
    ...
    kind: 'update',
    preview:
      'Your Q4 2025 cash dividend of $0.18/share will be deposited on Jan 24. DRIP participation is currently on.',
    tag: 'Dividend update',
    time: '2h',
    tone: 'info',
    unread: true,
  },
  {
    ...
    kind: 'notification',
    preview:
      'Action required — upload a W-9 to continue your transfer to Fidelity (case TR-120458).',
    tag: 'Action needed',
    time: '4h',
    tone: 'warning',
    unread: true,
  },
  {
    ...
    kind: 'communication',
    preview:
      '2026 annual meeting proxy is open. Cast your vote by Mar 12 · 5 proposals.',
  ...
  },
  {
    ...
    kind: 'draft',
    preview:
      'Re: Cost basis adjustment for Teagan Biosciences ESPP lot #3 — drafted but not sent.',
    tag: 'Draft',
    time: 'Yesterday',
    tone: 'neutral',
    unread: false,
  },
  {
    ...
    kind: 'notification',
    preview:
      'Your address change has been verified and applied to all 4 issuer ledgers.',
    tag: 'Account',
    time: '3d',
    tone: 'neutral',
    unread: false,
  },
  {
    ...
    kind: 'communication',
    preview:
      'Lock-up release schedule updated: 2,500 sh unlock on Mar 19, 2026.',
...
export function InboxTabs() {
  ...
  return (
    <div>
      <div className='px-1'>
        <div
          ...
          onClick={(e) => {
            ...
            if (target) {
              setActive(
                target.getAttribute('data-tab') as (typeof filters)[number]['id'],
              )
            }
          }}
          role='tablist'
      ...
      <ul className='mt-2 flex flex-col divide-y divide-[color:var(--color-line)]'>
        {visible.map((m) => (
          <li
            className={`flex items-start gap-3 px-1 py-3 cursor-pointer rounded-[6px] hover:bg-[color:var(--color-surface-2)] ${
              m.unread ? '' : 'opacity-85'
            }`}
            key={m.id}
          >
            <Avatar
              name={m.from}
              size={30}
              tone={m.channel === 'proxi' ? 'brand' : 'neutral'}
            />
            <div className='min-w-0 flex-1'>
              <div className='flex items-center gap-2'>
                ...
                <span className='ml-auto shrink-0 text-[11.5px] text-[color:var(--color-ink-500)]'>
                  {m.time}
                </span>
              </div>
              <p className='mt-1 line-clamp-2 text-[12.5px] text-[color:var(--color-ink-600)]'>
                {m.preview}
              </p>
              {m.kind === 'draft' && (
